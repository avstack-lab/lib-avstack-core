...
def test_JVC_linear_sum():
    A = np.array([[np.inf, -10, -1], [np.inf, -1, -30], [np.inf, 10, 10]])
    assign_sol = assignment.gnn_single_frame_assign(A, algorithm="JVC")
    assert assign_sol.cost == -40
    assert assign_sol.same(
        ds.OneEdgeBipartiteGraph({0: 1, 1: 2}, A.shape[0], A.shape[1], -40)
    )
    ...
    assert assign_sol.assignment_tuples == tuple(
        sorted(((1, 0), (3, 1), (4, 2), (0, 3)))
    )
...
def test_pda_assignment():
    ...
    gate_map = {
        c: [r[0] for r in np.argwhere(A[:, c] < np.inf)] for c in range(A.shape[1])
    }
    d2_map = {c: {r: A[r, c] for r in gate_map[c]} for c in gate_map}
    S = np.diag(2 * np.ones((3,)))
    S_map = {c: S for c in gate_map}
    assign_sol = assignment.pda_single_frame_assign(
        gate_map, d2_map, S_map, PD, BETA, A.shape[0], A.shape[1]
    )
...
def test_jpda_combinatorics():
    ...
    best_sols, probs = assignment._jpda_via_combinatorics(
        gate_map, p_H_partial, nrows, ncols
    )
...
def test_jpda_combinatorics_assignment():
    ...
    nrows = 3
    ncols = 2
    assign_sol = assignment.jpda_single_frame_assign(
        gate_map,
        d2_map,
        S_map,
        PD=PD,
