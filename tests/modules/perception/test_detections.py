...
from avstack.geometry import Box3D, StandardCoordinates, Translation, NominalOriginStandard
...
def test_detection_container():
    ...
    alg_ID = 0
    alg_name = 'detector-1'
    dets = [detections.CentroidDetection(alg_name, np.random.randn(3), obj_type='Car') for _ in range(4)]
...
def test_centroid_detection():
    ...
    d = detections.CentroidDetection(source_identifier=alg_name, centroid=centroid, obj_type='Car')
...
def test_lane_line_in_space_detection():
    ...
    lc2, lateral_offset, yaw_offset = lane_left.compute_center_lane_and_offset(lane_right)
