...
from avstack.datastructs import DataManager, DataContainer

sys.path.append('tests/')
from utilities import get_test_sensor_data, get_ego, get_object_local
...
def make_kitti_tracking_data(dt=0.1, n_frames=10, n_targs=4):
    ...
    for i in range(n_frames):
        ...
        dets_class = []
        for det in dets:
            det.box3d.t[2] += t * det.velocity.vector[2] # camera coordinates with z forward
...
def make_kitti_2d_3d_tracking_data(dt=0.1, n_frames=10, n_targs=4):
    ...
    for i, dets_3d in enumerate(dets_3d_all):
        d_2d = [BoxDetection('detector-2d', d.box.project_to_2d_bbox(camera_calib), d.obj_type) for d in dets_3d]
        dets_2d_all.append(DataContainer(i, dets_3d.timestamp, d_2d, source_identifier=name_2d))
...
def test_basic_joint_box_tracker():
    n_targs = 4
    dets_2d_all, dets_3d_all = make_kitti_2d_3d_tracking_data(n_frames=10, n_targs=n_targs)
...
def test_eagermot_fusion_kitti():
    dets_2d_all, dets_3d_all = make_kitti_2d_3d_tracking_data(n_frames=10)
    tracker_base = avstack.modules.tracking.tracker3d.libraries.EagerMOT.model.EagerMOT(framerate=10)
    for dets_2d, dets_3d in zip(dets_2d_all, dets_3d_all):
        # add false positives
        b2d = BoxDetection(name_2d, bbox.Box2D([10,20,20,30], camera_calib))
        b3d = BoxDetection(name_3d, bbox.Box3D([1,1,1,[10,-20,2],np.quaternion(1)], lidar_calib.origin))
        dets_2d.append(b2d)
        dets_3d.append(b3d)
...
def test_eagermot_associations():
    ...
    for frame, (dets_2d, dets_3d) in enumerate(zip(dets_2d_all, dets_3d_all)):
        ...
        if i == 0:
            ...
            b3d = BoxDetection(name_2d, bbox.Box3D([1,1,1,[19,-20,2],np.quaternion(1)], lidar_calib.origin))
            dets_2d.append(b2d)
            dets_3d.append(b3d)

        # test association
        lone_2d, lone_3d, fused_instances = trk_base.fusion(dets_2d, dets_3d)
        assign_1, lone_fused_1, lone_3d_1, lone_track_1 = trk_base.tracker._first_data_association(lone_3d, fused_instances)
        assign_2, lone_fused_2, lone_2d_2, lone_track_2 = trk_base.tracker._second_data_association(lone_2d, lone_fused_1, lone_track_1)
        # raise
        if i == 0:
            assert len(lone_track_1) == len(lone_track_2) == len(assign_1) == len(assign_2) == 0
        else:
        ...
        tracks = tracker(frame, dets_2d, dets_3d, 'tracker-1')
        if i == 1:
            assert len(tracks) == tracker.n_tracks_confirmed == 0  # due to confirmation time
        elif i > 3:
