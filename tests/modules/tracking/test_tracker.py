...
from avstack.geometry import bbox, Position, Attitude, ReferenceFrame, GlobalOrigin3D
...
def test_groundtruth_tracking():
    ...
    tracks = tracker(t=timestamp, frame=frame, detections=None, platform=platform,
                     ground_truth=ground_truth, identifier="tracker-1")
    ...
    for i in range(n_frames):
        ...
        for det in dets:
            ...
            if det_type == 'box':
                ...
                razelrrt = xyzvel_to_razelrrt(np.array([*det.box3d.t.x, *det.velocity.x]))
...
def make_kitti_2d_3d_tracking_data(dt=0.1, n_frames=10, n_targs=4):
    ...
    for i, dets_3d in enumerate(dets_3d_all):
        d_2d = [
            BoxDetection(
                "detector-2d", d.box.project_to_2d_bbox(camera_calib), reference, d.obj_type
            )
            for d in dets_3d
        ]
        dets_2d_all.append(
...
def test_razel_tracker_3d():
    ...
    dets_3d_all = make_kitti_tracking_data(dt=dt, n_frames=20, n_targs=4, det_type='razel')
    tracker = tracking.tracker3d.BasicRazelTracker(threshold_coast=5)
    for frame, dets_3d in enumerate(dets_3d_all):
        tracks = tracker(
            t=frame*dt, frame=frame, detections=dets_3d, platform=platform, identifier="tracker-1"
        )
...
def test_inline_razelrrt_tracker_3d():
    ...
    for frame in range(nframes):
        ...
        dets = [RazelRrtDetection(
            razelrrt=xyzvel_to_razelrrt(target_xyzvel), source_identifier='radar', reference=platform,
              obj_type=None)]
...
def test_razelrrt_tracker_3d():
    dt = 0.10
    platform = GlobalOrigin3D
    dets_3d_all = make_kitti_tracking_data(dt=dt, n_frames=20, n_targs=5, det_type='razelrrt')
    tracker = tracking.tracker3d.BasicRazelRrtTracker(threshold_coast=5)
    for frame, dets_3d in enumerate(dets_3d_all):
        tracks = tracker(
            t=frame*dt, frame=frame, detections=dets_3d, platform=platform, identifier="tracker-1"
...
def test_basic_box_tracker_2d():
    ...
    for frame, dets_2d in enumerate(dets_2d_all):
        tracks = tracker(
            t=frame*dt, frame=frame, detections=dets_2d, platform=platform, identifier="tracker-1"
        )
    assert len(tracker.tracks_active) == len(dets_3d_all[-1])
    for i, trk in enumerate(tracker.tracks_active):
        for det in dets_2d_all[-1]:
            if np.linalg.norm(trk.box2d.center - det.box.center) < 25:  # NOTE: this is a little high
                break
        else:
            raise
...
def test_eagermot_fusion_kitti():
    ...
    for dets_2d, dets_3d in zip(dets_2d_all, dets_3d_all):
        ...
        # add false positives
        b2d = BoxDetection(name_2d, bbox.Box2D([10, 20, 20, 30], camera_calib), camera_calib.reference)
        pos = Position(np.array([10, -20, 2]), lidar_calib.reference)
        rot = Attitude(np.quaternion(1), lidar_calib.reference)
        b3d = BoxDetection(name_3d, bbox.Box3D(pos, rot, [1, 1, 1]), lidar_calib.reference)
        dets_2d.append(b2d)
        dets_3d.append(b3d)
        lone_2d, lone_3d, fused_instances = tracker_base.fusion(dets_2d, dets_3d, platform)
...
def test_eagermot_associations():
    ...
    for frame, (dets_2d, dets_3d) in enumerate(zip(dets_2d_all, dets_3d_all)):
        # add false positives
        if i == 0:
            b2d = BoxDetection(name_3d, bbox.Box2D([10, 20, 20, 30], camera_calib), camera_calib.reference)
...
def test_box_tracker_moving_reference():
    ...
    for i in range(frames):
        reference = ReferenceFrame(box1.reference.x + v0*dt*i, box1.reference.q, GlobalOrigin3D)
        dets = [BoxDetection('boxes', box1.change_reference(reference, inplace=False), reference)]
