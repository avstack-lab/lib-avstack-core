...
def get_obj_vector_relative(ego, d, vector, obj_type="car"):
    ...
    box.t = pos
    obj.set(
        t=ego.t, position=pos, box=box, velocity=np.zeros((3,)), attitude=ego.attitude
    )
...
def get_obj_forward_relative(ego, d_forward, obj_type="car"):
    return get_obj_vector_relative(
        ego, d_forward, ego.attitude.forward_vector, obj_type=obj_type
    )


def get_obj_left_relative(ego, d_lateral, obj_type="car"):
    return get_obj_vector_relative(
        ego, d_lateral, ego.attitude.left_vector, obj_type=obj_type
    )
...
def test_rss_car():
    if safety.use_rss:
        ...
        rss_eval = safety.RssEvaluator()
        assert (
            rss_eval._rss_object(ego, obj).objectType
