...
def test_same_intersection_union():
    q = tforms.transform_orientation([0,0,1.2], 'euler', 'quat')
    box1 = bbox.Box3D([3, 4, 5, [3.4, 5.4, -10.2], q], origin=NominalOriginStandard)
    assert(abs(bbox.box_intersection(box1.corners, box1.corners) -
                bbox.box_union(box1.corners, box1.corners) < 1e-8))
...
def test_2d_box_valid():
    ...
    box = bbox.Box2D([-10, 20, 50, 60], cam_calib)
    assert box.squeeze(im_h, im_w) != box
    assert not box._x_valid(im_w) and box._y_valid(im_h) and not box.check_valid(im_h, im_w)
...
def test_io_2d_bbox():
    ...
    img_shape = (256, 1024)
    cam_calib = calibration.CameraCalibration(NominalOriginStandard, P, img_shape=img_shape)
