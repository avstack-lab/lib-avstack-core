...
def test_rotation_conversion():
    D = conversions.transform_orientation([0, 0, 0], "euler", "DCM")
...
def test_xyzvel_to_razelrrt():
    ...
    razelrrt_cal = conversions.xyzvel_to_razelrrt(xyzvel)
    assert np.allclose(razelrrt_cal, razelrrt_exp)


def test_razelrrt_to_xyzvel():
    xyzvel = np.random.randn(6)
    razelrrt = conversions.xyzvel_to_razelrrt(xyzvel)
    xyzvel_recon = conversions.razelrrt_to_xyzvel(razelrrt)
