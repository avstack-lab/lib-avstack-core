...
from avstack.geometry import Origin, Transform, Translation, Rotation, StandardCoordinates, CameraCoordinates
from avstack.geometry import NominalOriginCamera, NominalOriginStandard, get_transform_from_line
...
def test_origin():
    ...
    assert np.all(origin.matrix == np.array([[1, 0, 0, -1],
                                             [0, 1, 0, -2],
                                             [0, 0, 1, -3],
                                             [0, 0, 0, 1]]))
...
def test_translation_nonsame():
    ...
    assert np.isclose(trans_1.distance(trans_2), np.linalg.norm(v1 - R_cam_to_stan@v2))
...
def test_translation_change_origin():
    ...
    assert np.allclose(trans_1.vector, quaternion.rotate_vectors(q2, v1 - x2), atol=1e-4)
...
def test_rotation_nonsame():
    ...
    assert quaternion.allclose((rot_1 @ rot_2).q, q1 * q2 * q_stan_to_cam, rtol=1e-5, atol=1e-8)
...
def test_transform_1():
    ...
    q1 = tforms.transform_orientation([0, 1.1, -2], 'euler', 'quat')
    q2 = tforms.transform_orientation([1, -1.1, -1], 'euler', 'quat')
    transf_1 = Transform(Rotation(q1, origin=NominalOriginStandard),
        Translation(v1, origin=NominalOriginStandard))
    transf_2 = Transform(Rotation(q2, origin=NominalOriginStandard),
...
def test_transform_change_origin():
    v1 = np.array([10, -2.2, 1.3])
    q1 = tforms.transform_orientation([0, 1.1, -2], 'euler', 'quat')
    T_1 = Transform(Rotation(q1, origin=NominalOriginStandard),
        Translation(v1, origin=NominalOriginStandard))
...
def test_transform_compose_associative():
    ...
    T_1 = Transform(Rotation(q1, origin=NominalOriginStandard),
        Translation(v1, origin=NominalOriginStandard))
    T_2 = Transform(Rotation(q2, origin=NominalOriginStandard),
        Translation(v2, origin=NominalOriginStandard))
    ...
    assert np.allclose(T_1.rotation.R, tforms.transform_orientation([0, 1.1, -2], 'euler', 'DCM'))
    assert np.allclose(T_1.translation.vector, np.array([10, -2.2, 1.3]))
    assert np.allclose(T_2.rotation.R, tforms.transform_orientation([1, -1.1, -1], 'euler', 'DCM'))
...
def get_T_for_compose_mixed():
    ...
    T_1 = Transform(Rotation(q1, origin=NominalOriginStandard),
        Translation(v1, origin=NominalOriginStandard))
    T_2 = Transform(Rotation(q2, origin=NominalOriginCamera),
