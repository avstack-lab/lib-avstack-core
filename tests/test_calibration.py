...
from avstack.geometry import NominalTransform, Rotation, Translation, Transform, Origin
from avstack.geometry import NominalOriginCamera, NominalOriginStandard
from avstack.geometry import CameraCoordinates, StandardCoordinates, R_cam_to_stan, R_stan_to_cam, q_stan_to_cam
...
def test_project_to_camera_halves_same_origin():
    ...
    pts_all = 2*(np.random.rand(1000, 3)-0.5) * np.array([10, 10, 2]) + np.array([40, 0, 0])
...
def test_calib_angles():
    ...
    img_shape = [900, 1600, 3]
    fov_h = 90*np.pi/180
    calib_camera = calibration.CameraCalibration(O2, P, img_shape,
        fov_horizontal=fov_h, fov_vertical=None, square_pixels=True)
