...
def get_reference_from_line(line):
    ...
    if int(items[1]) == 0:
        ...
        idx = 2
        x = np.array([float(r) for r in items[idx:idx+3]]); idx += 3
        q = np.quaternion(*[float(w) for w in items[idx:idx+4]]); idx += 4
        v = np.array([float(r) for r in items[idx:idx+3]]); idx += 3
        acc = np.array([float(r) for r in items[idx:idx+3]]); idx += 3
        ang = np.quaternion(*[float(w) for w in items[idx:idx+4]]); idx += 4
        return ReferenceFrame(x=x, q=q, v=v, acc=acc, ang=ang,
                              reference=get_reference_from_line(" ".join(items[idx:])))
    

class ReferenceFrame:
    def __init__(self, x: np.ndarray, q: np.quaternion, reference: ReferenceFrame,
                 v: np.ndarray=np.zeros((3,),dtype=float), acc: np.ndarray=np.zeros((3,),dtype=float), 
                 ang: np.quaternion=np.quaternion(1), handedness='right', n_prec=8) -> None:
        self.n_prec = n_prec
    ...
    @property
    def fixed(self):
        return np.allclose(self.v, np.zeros((3,))) and \
               np.allclose(self.acc, np.zeros((3,))) and \
               np.allclose(self.ang.vec, np.zeros((3,)))
    ...
    def __hash__(self):
        ...
        if self._hash is None:
            self._hash = hash(self.x.tobytes() + self.q.vec.tobytes() + 
                              self.v.tobytes() + self.acc.tobytes() + 
                              self.ang.tobytes())
    ...
    def allclose(self, other):
        ...
        return np.allclose(diff.x, np.zeros(3,)) and \
               np.allclose(diff.q.vec, np.zeros(3,)) and \
               np.allclose(diff.v, np.zeros(3,)) and \
               np.allclose(diff.acc, np.zeros(3,)) and \
               np.allclose(diff.ang.vec, np.zeros(3,))
    ...
    def integrate(self, start_at: ReferenceFrame):
        ...
        return ReferenceFrame(x=x_tot, q=q_tot, v=v_tot, acc=acc_tot, ang=ang_tot, reference=start_at)
    ...
    def differential(self, other: ReferenceFrame, in_self: bool=True):
        ...
        if in_self:
            ...
            if not (self.fixed and other.fixed):
                ...
                acc_self_to_other_in_self = q_mult_vec(A_2_self.q, acc_self_to_other_in_A)
                ang_self_to_other = A_2_other.ang * A_2_self.ang.conjugate()  # TODO this is definitely wrong
                ref = ReferenceFrame(x=x_self_to_other_in_self, q=q_self_to_other,
                                     v=v_self_to_other_in_self, acc=acc_self_to_other_in_self,
                                     ang=ang_self_to_other, reference=self)
            else:
                ref = ReferenceFrame(x=x_self_to_other_in_self, q=q_self_to_other,
                                     reference=self)
            return ref
            ...
            if not (self.fixed and other.fixed):
                ...
                acc_other_to_self_in_other = q_mult_vec(A_2_other.acc, acc_other_to_self_in_A)
                ang_other_to_self = A_2_self.ang * A_2_other.ang.conjugate()  # TODO this is definitely wrong
                ref = ReferenceFrame(x=x_other_to_self_in_other, q=q_other_to_self,
                                  v=v_other_to_self_in_other, acc=acc_other_to_self_in_other,
                                  ang=ang_other_to_self, reference=other)
            else:
                ref = ReferenceFrame(x=x_other_to_self_in_other, q=q_other_to_self,
                                     reference=other)
            return ref
    ...
    def engross(self):
        ...
        return ReferenceFrame(x=np.zeros(3,), q=np.quaternion(1), reference=self, n_prec=self.n_prec)

    def format_as_string(self):
        if self.reference is None:
            return f"reference 0 global"
        else:
            return f"reference {self.level} {self.x[0]} {self.x[1]} {self.x[2]} " \
...
GlobalOrigin3D = ReferenceFrame(x=np.zeros((3,)), q=np.quaternion(1), v=np.zeros((3,)),
                                acc=np.zeros((3,)), ang=np.quaternion(1),
                                reference=None, handedness='right')


class Vector():
    def __init__(self, x: np.ndarray, reference: ReferenceFrame, n_prec: int=8) -> None:
        self.n_prec = n_prec
    ...
    def change_reference(self, reference: ReferenceFrame, inplace: bool, angle_only: bool=False):
...
class VectorHeadTail():
    def __init__(self, head: np.ndarray, tail: np.ndarray, reference: ReferenceFrame, n_prec: int=8) -> None:
        self.n_prec = n_prec
    ...
    def change_reference(self, reference: ReferenceFrame, inplace: bool, angle_only: bool=False):
        if inplace:
            self.head.change_reference(reference, inplace=inplace, angle_only=angle_only)
            self.tail.change_reference(reference, inplace=inplace, angle_only=angle_only)
        else:
            head = self.head.change_reference(reference, inplace=inplace, angle_only=angle_only)
            tail = self.head.change_reference(reference, inplace=inplace, angle_only=angle_only)
...
class Spherical(Vector):
    def __init__(self, x: np.ndarray, reference: ReferenceFrame, n_prec: int=8, wrapping='v1') -> None:
        """
...
class Rotation():
    ...
    def q(self, q):
        ...
        self._q = np.quaternion(np.round(q.w, self.n_prec), 
                               *fastround(q.vec, self.n_prec, y))
