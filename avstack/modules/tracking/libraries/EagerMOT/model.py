...
from avstack.environment.objects import IOU_2d
...
class EagerMOTTracking:
    ...
    def tracks_confirmed(self):
        if self.plus:
            return {
                t_id: t
                for t_id, t in self._tracks_confirmed_all.items()
                if t.box3d_initialized
                and t.box2d_confirmed
                and (t.box2d_n_confirmed >= self.n_box_confirmed)
                and (t.box3d_n_confirmed >= self.n_box_confirmed)
                and (t.box3d_coast <= self.n_joint_coast)
                and (t.box2d_coast <= self.n_joint_coast)
            }
        else:
            return {
                t_id: t
        ...
        for det_2d, track in assign_2:
            if isinstance(det_2d, JointBoxDetection):
                ...
                raise NotImplementedError(
                    f"Update using {type(det_2d)} not implemented -- {det_2d}"
                )
            ...
            for d in ds:
                for j, t in enumerate(lone_tracks):
                    try:
                        t_box = (
                            t.box2d
                            if t.box2d is not None
                            else t.box3d.project_to_2d_bbox(d.data.calibration)
                        )
                        ...
                        t_box = (
                            t.box2d
                            if t.box2d is not None
                            else t.box3d.project_to_2d_bbox(d.box2d.calibration)
                        )
...
class EagerMOTFusion:
    ...
    def __call__(self, detections_2d, detections_3d, platform):
        ...
        """
        assert isinstance(
            detections_2d, (DataContainer, dict)
        ), f"{type(detections_2d)}"
        assert isinstance(
            detections_3d, (DataContainer, dict)
        ), f"{type(detections_3d)}"
        if isinstance(detections_2d, dict):
            assert (
                len(detections_2d) == 1
            ), f"{detections_2d} For now can only do 1 sensor"
        ...
        if isinstance(detections_3d, dict):
            assert (
                len(detections_3d) == 1
            ), f"{detections_3d} For now can only do 1 sensor"
        ...
        for j, det_3d in enumerate(detections_3d):
            for i, det_2d in enumerate(detections_2d):
                if det_2d.data.calibration != last_calib:
                    det_3d_in_2d = det_3d.data.project_to_2d_bbox(
                        det_2d.data.calibration
                    )
                else:
